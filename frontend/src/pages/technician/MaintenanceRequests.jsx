...
const MaintenanceRequests = () => {
    ...
    const fetchMaintenanceRequests = async () => {
        setLoading(true);
        try {
            // Fetch only MAINTENANCE type bookings
            const response = await api.get('/bookings/my', {
                params: { bookingType: 'MAINTENANCE', size: 100 }
            });
            const maintenanceBookings = (response.data.content || response.data).filter(
                booking => booking.bookingType === 'MAINTENANCE'
            );
            setRequests(maintenanceBookings);
        } catch (error) {
    ...
    const handleStartMaintenance = async (bookingId) => {
        setProcessing(true);
        try {
            const response = await api.post(`/maintenance/${bookingId}/start-maintenance`);
            if (response.data) {
    ...
    const handleCompleteMaintenance = async (bookingId) => {
        setProcessing(true);
        try {
            const response = await api.post(`/maintenance/${bookingId}/complete-maintenance`);
            if (response.data) {
    ...
    const handleRequestExtension = async () => {
        ...
        setProcessing(true);
        try {
            const response = await api.post(`/maintenance/${selectedRequest.id}/request-extension?days=${extensionDays}`);
            if (response.data) {
                                        ...
                                        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                            ...
                                            <div className="flex flex-row lg:flex-col gap-2">
                                                {statusConfig.actions.includes('start') && (
                                                    <button
                                                        onClick={() => handleStartMaintenance(request.id)}
                                                        disabled={processing}
                                                        className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-sm font-semibold hover:bg-blue-100 transition"
                                                    >
                                                ...
                                                <button
                                                    onClick={() => {
                                                        setSelectedRequest(request);
                                                        // Show details modal (optional)
                                                    }}
                                                    className="flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-100 transition"
                                                >
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                                <p className="text-xs text-blue-700">
                                    ...
                                    The resource will remain in maintenance mode until a decision is made.
                                </p>
                            </div>
                        </div>
